...
function get_reply_post($connect, $parent_id = 0) {
 ...
 INNER JOIN tbl_users ON
 tbl_posts.post_creator_id = tbl_users.id
 WHERE parent_post_id = '$parent_id' ORDER BY post_id ASC
 ";
  $output = '';
  $statement = $connect->prepare($query);
  $statement->execute();
...
// $id = $_SESSION['id'];
// $sql = "SELECT * FROM tbl_posts INNER JOIN tbl_users ON tbl_users.id = tbl_posts.post_creator_id WHERE tbl_posts.image != '' OR null";
// $statement = $connect->prepare($sql);
// $statement->execute();
// $result = $statement->fetchAll();
...
// foreach ($result as $row){
// $image = $row['image'];
// echo "<div class='single-img'><img style='width: 100px; height: 100px' class='img".$row['post_id']."' src='images/".$image."'></div>";  
// }
// } else {
//   echo "<div class='small text-muted text-left'>".strtok($name, " ")." has not posted any images yet</small></div>";
// }
